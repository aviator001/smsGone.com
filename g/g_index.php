<?
	include "/home/smsgone/public_html/g/utils.class.php";
	$u = new utils();
	$is = $u->is_mobile();
?>
<!DOCTYPE html>
...
<? if ($is) { ?>
		...
		function page_init() {
			...
			document.getElementById('msg_btn').style.display='none'
			document.getElementById('alpha').style.display='none'
		}

		
		function dispatch(key)	{
			var url = '../g/g_descramble.php?bin_user_token=<?=($_GET[i]);?>&key=' + key
			console.log(url)
					var request = $.ajax({
						url: url,
						type: "GET",
...
//							alert(msg)
							g=setTimeout("garbage('<?=($_GET[i]);?>')",1000)
							selfD=msg.split('|')[0]
...
<? } ?>
		...
		function fade(objID) {
			...
			if (op[objID] >= 0) {
				...
				v[objID]=1
				el[objID].style.opacity=op[objID]
				if (xctr==0) g=setTimeout('garbage()',1000)
			}
		}
		
		function garbage(token)	{
			var url = 'g_garbage.php?bin_user_token='+token
			console.log(url)
			var request = $.ajax({
				url: url,
				type: "GET",
